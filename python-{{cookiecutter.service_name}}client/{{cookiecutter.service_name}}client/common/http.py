...
USER_AGENT = 'python-{{cookiecutter.service_name}}client'
...
class VersionNegotiationMixin(object):
    def negotiate_version(self, conn, resp):
        ...
        if not max_ver:
            ...
            if self.os_{{cookiecutter.service_name}}_api_version:
                base_version = ("/v%s" %
                                str(self.os_{{cookiecutter.service_name}}_api_version).split('.')[0])
            else:
        ...
        if self.api_version_select_state == 'user':
            raise exc.UnsupportedVersion(textwrap.fill(
                ...
                "%(max)s"
                % {'req': self.os_{{cookiecutter.service_name}}_api_version,
        ...
        negotiated_ver = min(self.os_{{cookiecutter.service_name}}_api_version, max_ver)
        ...
        self.os_{{cookiecutter.service_name}}_api_version = negotiated_ver
    ...
    def _generic_parse_version_headers(self, accessor_func):
        min_ver = accessor_func('X-OpenStack-{{cookiecutter.service_name|capitalize}}-API-Minimum-Version',
                                None)
        max_ver = accessor_func('X-OpenStack-{{cookiecutter.service_name|capitalize}}-API-Maximum-Version',
                                None)
    ...
    def wrapper(self, url, method, **kwargs):
        ...
        for attempt in range(1, num_attempts + 1):
            try:
                ...
                msg = ("Error contacting {{cookiecutter.service_name|capitalize}} server: %(error)s. "
...
class HTTPClient(VersionNegotiationMixin):
    ...
    def __init__(self, endpoint, **kwargs):
        ...
        self.os_{{cookiecutter.service_name}}_api_version = kwargs.get('os_{{cookiecutter.service_name}}_api_version',
                                                DEFAULT_VER)
    ...
    def _http_request(self, url, method, **kwargs):
        ...
        if self.os_{{cookiecutter.service_name}}_api_version:
            kwargs['headers'].setdefault('X-OpenStack-{{cookiecutter.service_name|capitalize}}-API-Version',
                                         self.os_{{cookiecutter.service_name}}_api_version)
        if self.auth_token:
        ...
        try:
            ...
            if resp.status == 406:
                ...
                kwargs['headers']['X-OpenStack-{{cookiecutter.service_name|capitalize}}-API-Version'] = (
                    negotiated_ver)
...
class SessionClient(VersionNegotiationMixin, adapter.LegacyJsonAdapter):
    ...
    def _http_request(self, url, method, **kwargs):
        ...
        if getattr(self, 'os_{{cookiecutter.service_name}}_api_version', None):
            kwargs['headers'].setdefault('X-OpenStack-{{cookiecutter.service_name|capitalize}}-API-Version',
                                         self.os_{{cookiecutter.service_name}}_api_version)
...
def _construct_http_client(*args, **kwargs):
    ...
    if session:
        ...
        os_{{cookiecutter.service_name}}_api_version = kwargs.pop('os_{{cookiecutter.service_name}}_api_version',
                                           DEFAULT_VER)
        session_client = SessionClient(session=session,
                                       ...
                                       service_name=None,
                                       user_agent='python-{{cookiecutter.service_name}}client')
        # Append an {{cookiecutter.service_name}} specific variable to session
        session_client.os_{{cookiecutter.service_name}}_api_version = os_{{cookiecutter.service_name}}_api_version
