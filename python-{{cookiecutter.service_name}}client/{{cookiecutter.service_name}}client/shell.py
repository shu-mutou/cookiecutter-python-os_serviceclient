...
"""
Command-line interface to the OpenStack {{cookiecutter.service_name|upper}} API.
"""
...
from {{cookiecutter.service_name}}client import client as cli
from {{cookiecutter.service_name}}client.common import http
from {{cookiecutter.service_name}}client.openstack.common. import _
...
class {{cookiecutter.service_name|capitalize}}Shell(object):
    ...
    def get_base_parser(self):
        parser = argparse.ArgumentParser(
            prog='{{cookiecutter.service_name}}',
            description=__doc__.strip(),
            epilog='See "{{cookiecutter.service_name}} help COMMAND" '
        ...
        parser.add_argument('--version',
                            action='version',
                            version={{cookiecutter.service_name}}client.__version__)

        parser.add_argument('--debug',
                            default=bool(cliutils.env('{{cookiecutter.service_name|upper}}CLIENT_DEBUG')),
                            action='store_true',
                            help='Defaults to env[{{cookiecutter.service_name|upper}}CLIENT_DEBUG]')
        ...
        parser.add_argument('--{{cookiecutter.service_name}}-url',
                            default=cliutils.env('{{cookiecutter.service_name|upper}}_URL'),
                            help='Defaults to env[{{cookiecutter.service_name|upper}}_URL]')

        parser.add_argument('--{{cookiecutter.service_name}}_url',
                            help=argparse.SUPPRESS)

        parser.add_argument('--{{cookiecutter.service_name}}-api-version',
                            default=cliutils.env(
                                '{{cookiecutter.service_name|upper}}_API_VERSION', default='1'),
                            help='Accepts 1.x (where "x" is microversion) '
                                 'or "latest", Defaults to '
                                 'env[{{cookiecutter.service_name|upper}}_API_VERSION] or 1')

        parser.add_argument('--{{cookiecutter.service_name}}_api_version',
                            help=argparse.SUPPRESS)
        ...
        parser.add_argument('--max-retries', type=int,
                            ...
                            'Defaults to env[{{cookiecutter.service_name|upper}}_MAX_RETRIES] or %d. '
                            'Use 0 to disable retrying.'
                            % http.DEFAULT_MAX_RETRIES,
                            default=cliutils.env(
                                '{{cookiecutter.service_name|upper}}_MAX_RETRIES',
        ...
        parser.add_argument('--retry-interval', type=int,
                            ...
                            'Defaults to env[{{cookiecutter.service_name|upper}}_RETRY_INTERVAL] or %d.'
                            % http.DEFAULT_RETRY_INTERVAL,
                            default=cliutils.env(
                                '{{cookiecutter.service_name|upper}}_RETRY_INTERVAL',
    ...
    def _check_version(self, api_version):
        if api_version == 'latest':
            ...
            try:
                ...
                versions = ()
            if len(versions) == 1:
                # Default value of {{cookiecutter.service_name}}_api_version is '1'.
                # If user not specify the value of api version, not passing
                # headers at all.
                os_{{cookiecutter.service_name}}_api_version = None
            elif len(versions) == 2:
                os_{{cookiecutter.service_name}}_api_version = api_version
                # In the case of '1.0'
                if versions[1] == 0:
                    os_{{cookiecutter.service_name}}_api_version = None
            else:
            ...
            return (api_major_version, os_{{cookiecutter.service_name}}_api_version)
    ...
    def main(self, argv):
        ...
        (api_major_version, os_{{cookiecutter.service_name}}_api_version) = (
            self._check_version(options.{{cookiecutter.service_name}}_api_version))
        ...
        if not (args.os_auth_token and (args.{{cookiecutter.service_name}}_url or args.os_endpoint)):
        ...
        endpoint = args.{{cookiecutter.service_name}}_url or args.os_endpoint
        service_type = args.os_service_type or 'baremetal'
        project_id = args.os_project_id or args.os_tenant_id
        project_name = args.os_project_name or args.os_tenant_name

        if (args.os_auth_token and (args.{{cookiecutter.service_name}}_url or args.os_endpoint)):
            kwargs = {
        ...
        kwargs['os_{{cookiecutter.service_name}}_api_version'] = os_{{cookiecutter.service_name}}_api_version
        ...
        client = cli.Client(api_major_version, endpoint, **kwargs)
...
def main():
    try:
        {{cookiecutter.service_name|capitalize}}Shell().main(sys.argv[1:])
    except KeyboardInterrupt:
        print("... terminating {{cookiecutter.service_name|upper}} client", file=sys.stderr)
        sys.exit(130)
